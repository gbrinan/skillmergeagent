...
- **제거 우선.** 두 스킬이 같은 일을 하면 하나로 줄인다. 아무도 답 못 하는 항목은 뺀다. 의견이 갈리면 빼는 쪽이 기본값이다. 고칠 것이 없는 한 판은 아무것도 바꾸지 않는다.
...
여섯 스킬 모두 model-invoked다. 사용자도 이름으로 부를 수 있다. 어느 것도 파일을 지우지 않고, 파일을 바꾸는 스킬은 `skillmerge` 하나뿐이며 계획 보고와 명시적 승인 뒤에만 바꾼다. `intake`는 읽기 전용, `mergechk`는 팩 안에 `DECISIONS.md`·`REMEDIATION.md`만 쓴다. 나머지는 기획서·계약·planning 파일을 만든다.
