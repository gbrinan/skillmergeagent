...
스킬 파일이나 정리된 자료가 없어도 이렇게 요청할 수 있다:

> 주간 보고 업무를 워크플로우로 만들고 싶어. 필요한 정보가 없으면 한 번에 하나씩 물어보면서 만들어줘.

`askflow`는 대화와 자료에서 이미 아는 값을 채우고 다음 설계 결정을 막는 것만 묻는다.

| 필요한 정보 | 예시 | 언제 필요한가 |
| --- | --- | --- |
| 목적·범위 | 보고 준비 시간을 줄이고, 자동 발송은 제외 | 첫 초안 |
| 시작·입력 | 매주 금요일, 사용자가 전달한 업무 메모 | 첫 초안 |
| 결과·완료 기준 | 팀장이 읽을 요약; 담당·기한 누락 없음 | 첫 초안 |
| 단계·담당 | 정리 → 초안 → 검토; AI와 사람의 역할 | 흐름 확정 |
| 판단·예외 | 입력 누락 시 되묻기, 발송 전 확인 | 흐름 확정 |
| 환경·권한 | 쓸 도구, 접근 여부, 저장/발송 범위 | 해당 단계 확정 |
| 표·필드·원본 | 실제 시트/DB를 쓰는 경우만 | 해당 업무만 |

순서가 없어도 에이전트가 추정 초안을 제안한다. 답을 모르면 미확인으로 남겨 나머지를 진행하고, 중요한 예외나 충돌은 삭제하지 않는다. 표를 쓰지 않는 업무에 엑셀이나 칸 이름을 요구하지 않는다. 사분면·폴더 배치는 사용자에게 묻지 않는다.

기본 결과는 `agent-plan.md` 하나다. 단계마다 **담당·입력·행동·출력·다음 조건**, 그리고 **예외·정지·완료 기준·미결·다음 질문**이 남는다. 다음 대화에서는 이 문서에서 이어간다. 실행에 필요한 결정이 모두 정해지면 질문을 끝내고 **설계 완료, 실행 미검증**으로 전달한다. 실행 팩이나 계약을 요청했거나 기존 팩을 수정할 때는 아래의 8절 기획서와 계약 게이트를 적용한다.

## 생성 후 한 번 더 다듬기

생성된 에이전트의 불필요한 설명과 중복 사실을 한 번 더 검토하는 선택적 Codex Stop 훅을 제공한다. 복구 사본을 남기고, 사실별 정본·보존할 예외·수정안을 보여 준 뒤 승인된 부분만 다듬는다. 무변경이면 종료하며 미결을 감추거나 자동 반복하지 않는다. [설치·등록·결과 확인](docs/post-generation-hook.md), [정리 규칙 정본](docs/refine-agent.md).

## 용어

문답과 완성된 단계 표의 예시는 [자료 없이 시작하는 문답](./docs/workflow-dialogue.md)에 있다.
...
| **기획서** | `agent-plan.md`가 설계 정보의 원본(SSOT). 실행 팩에는 팀과 목적 · 역할 · 데이터 명세 · 시나리오 · 결과 반영 · 연동 맵 · 정지 지점 · 폴더 트리의 8절을 쓴다 |
...
| 🎤 **[askflow](./skills/depth/askflow/SKILL.md)** | 자료 없이도 시작해, 빠진 결정만 한 번에 하나씩 묻고 초안을 갱신하는 워크플로우 인터뷰 | 흐름 하나 | model | |
...
├── scripts/             ← 스킬 카탈로그 검증 · 생성 후 검토 훅
├── out/                 ← 로컬 합성 시연 산출물 (선택, Git 제외)
├── docs/                ← 점검 기준
