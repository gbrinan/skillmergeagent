...
### 생성 후 정리 훅 (2026-09-09)

- 로컬 OMO Stop 설정과 공식 https://learn.chatgpt.com/docs/hooks 확인: Stop의 decision=block/reason은 후속 턴 요청이며 stop_hook_active로 반복을 피한다. 호스트 trust와 활성화가 필요하다.
- SSOT 감사: 생성 종료점은 askflow·weave, 검토는 mergechk, 전역 설명은 CLAUDE다. 정리 지시문을 각각 복사하지 않고 docs/refine-agent.md를 정본으로 새로 두고 연결한다. 기존 자기완결 스킬 규칙과 계약 파서 필드는 지우지 않는다.
- 파일명·본문 검색 두 방식으로 확인: 기존 후처리 훅 없음. 기획서/계약/개별 행동의 정본 구분은 기존 규약과 일치한다. 의미가 다른 예외·임계값은 문장 유사도만으로 합칠 수 없다.
- 범위를 디렉터리 전체로 추측하지 않는다. 세션 ID와 명시적 Markdown 파일 목록으로 등록한다. 훅은 복구 스냅샷과 검토 요청만 만들고, 의미 편집은 감사안 승인 뒤 에이전트가 수행한다. 무변경·미완료·미결·검토 후 변경을 구분한다.

### 문답 기반 워크플로우 설계 (2026-09-09)

- 기존 intake는 자료가 얇으면 중단하고, 표 없는 입력도 표 정의를 우선 질문했다. askflow는 8절 기획서·계약·사분면을 필수로 요구해 파일 없는 시작을 지원하지 못했다.
- 필요한 정보는 목적/범위, 시작/입력, 결과/완료, 단계/담당, 판단/예외, 환경/권한이며 데이터 구조는 실제 표/DB가 있는 경우만이다. 스킬이 없는 단계는 사람 또는 미구현으로 구분한다.
- 설계와 실행 팩을 구분했다. 설계는 agent-plan.md 하나로 완성하고 팩 요청·기존 팩 수정에는 기존 계약 게이트를 유지한다. 병렬/하위 호출/반복을 표현 못 하는 계약에 강제로 맞추지 않는다.
- 미확인과 해당 없음을 구분하고, 충돌은 삭제하지 않고 실행 보류로 남긴다. 질문 한 개마다 초안을 갱신하고 재개할 질문을 기획서에 남긴다.
- 이번 변경은 질문/설계 기능 확장이다. 분류기·CI 문자열 검사 등 앞선 감사의 별도 이슈는 변경하지 않았다.
...
### 실측 3a: 사용자의 실제 .claude/skills (2026-09-05)

제가 만든 자료가 아닌 첫 폴더다. 동기화된 스킬 10개 + session-start-hook 1개. 사람 인터뷰 없이 읽기 전용 부분만 돌렸다.

| 본 것 | 결과 |
| --- | --- |
| similarity 11개 | 합침 후보 0. 개인 도구 상자라 서로 다른 일이다 |
| flow 11개 | 간선 0, 고아 11, 카탈로그. readchk도 같은 판정 |
| 번들 `/mnt/skills`(41개)와 나란히 | **7개가 바이트까지 같은 복사본**(docx·pdf·pptx·xlsx·import-memory·morning·skill-creator). NVIDIA의 `nvidia-skill-finder`와 같은 종류의 진짜 중복 |
| 사용자 고유 스킬 3개 | backward-design-questions · grill-me · weekly-model-performance-report. 서로 부르지 않는다 |
| name ≠ 폴더 | `session-start-hook/`의 name이 `startup-hook-skill` |
| flow.py 오탐 | docx·pptx 본문의 XML 이름공간 17개를 없는 스킬로 냈다. 접두어 4자 이상, 코드 조각·태그 제외로 고침 |

배운 것: 실제 폴더에서 엔진이 낸 값어치는 통폐합이 아니라 **"이미 다른 데 있는 것"** 이었다. 공개 리포에서도 그랬고(NVIDIA 두 곳 복사) 개인 폴더에서도 그렇다(번들 복사 7개). 그리고 남이 만든 자료를 넣자마자 오탐이 나왔다. 제가 만든 자료로는 안 나오던 것이다. 남은 것은 결정이다: 복사본 7개가 버전을 고정하려는 의도인지, 그냥 쌓인 것인지는 사람만 안다.

### 흐름 추정 (2026-09-05)

"실제로 동작하는가"에 대한 답이 "검사기는 되고 통폐합 에이전트는 아직"이었다. 핵심 구멍 둘: 규격을 쓰는 공개 스킬이 0/692라 유사도의 주력 축이 헛돌고, 인터뷰가 "순서를 설계해 주세요"라 사람이 안 붙는다. 사용자가 "스킬들을 놓고 어떤 워크플로우를 만들 수 있는지 판별하는 기능"을 제안했고, 신호가 있는지부터 쟀다.

| 리포 | 스킬 | 서로 이름 부름 | 순서 신호 붙은 것 |
| --- | --- | --- | --- |
| superpowers | 14 | 21쌍 | 10쌍 |
| NVIDIA | 350 | 1003쌍 | 217쌍 |
| wshobson | 183 | 112쌍 | 13쌍 |
| anthropics | 20 | 0쌍 | 0쌍 (카탈로그) |

superpowers에서 사람 입력 없이 그 저장소의 실제 워크플로우(using-superpowers → brainstorming → writing-plans → executing-plans → finishing-a-development-branch, systematic-debugging → tdd·verification)가 나왔다. 두 구현이 독립적으로 같은 그림(간선 ±1)을 냈다.

배운 것: 워크플로우는 이미 스킬 본문에 적혀 있고 `next` 필드만 보던 엔진이 못 읽고 있었다. 규격은 입력이 아니라 출력이 돼야 한다. 첫 판의 오탐 둘(예시 나열, `✅ Good:` 서식 예 안의 REQUIRED SUB-SKILL)은 이름표로 고정했다. 관계 종류를 뭉개면 안 된다: 순서·하위스킬은 체인, 선행지식은 읽을 순서, 언급은 간선 아님. 알려진 누락: 순서 단어 없는 문장의 진짜 간선(`subagent-driven-development → requesting-code-review`).

이것이 실측 3을 가능하게 한다. 인터뷰 첫 질문이 "이 순서 맞습니까"가 된다.

평가 2개는 5/5였지만 채점자가 같은 지적을 했다. 고정 자료의 README가 기대 답을 적어 두면 에이전트가 본문을 읽지 않아도 통과한다. 실측 3이 값어치 있는 이유가 여기에도 있다. 정답이 없는 폴더에서만 이 스킬이 읽는지 안 읽는지 알 수 있다.
